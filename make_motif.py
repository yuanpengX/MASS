...
if True:
    ...
    tl.files.exists_or_mkdir(f'/data/group/m6A//{model}-spe')
    tl.files.exists_or_mkdir(f'/data/group/m6A/{model}-spe/{sample}/')
    ...
    for i in range(num_filter):
        plot_filter_logo(filter_out[:,:,i], filter_size, seqs, f'/data/group/m6A/{model}-spe/{sample}/filter%03d_logo'%(i),maxpct_t=0.5)
